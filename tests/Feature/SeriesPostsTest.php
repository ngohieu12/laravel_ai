...
class SeriesPostsTest extends TestCase
{
    ...
    public function test_a_post_can_join_a_series(): void
    {
        ...
        $this->assertSame('Học Laravel từ đầu', $post->series_title);
    ...
    public function test_joining_a_series_requires_a_part_number(): void
    {
        ...
        $this->actingAs($creator)
            ->post(route('dashboard.posts.store'), $this->payload(['series_part' => null]))
    ...
    {
        $creator = User::factory()->creator()->create();
        Post::factory()->for($creator, 'user')->inSeries('Học Laravel từ đầu', 1)->create();

        $this->actingAs($creator)
            ->post(route('dashboard.posts.store'), $this->payload(['series_part' => '1']))
            ->assertSessionHasErrors('series_part');

        // Cùng số phần nhưng khác chuỗi thì vẫn hợp lệ.
        $this->actingAs($creator)
            ->post(route('dashboard.posts.store'), $this->payload(['series_title' => 'Chuỗi khác', 'series_part' => '1']))
            ->assertRedirect(route('dashboard.posts.index'));

        $this->assertSame(1, Post::query()->where('series_title', 'Chuỗi khác')->count());
    }
    ...
    {
        $creator = User::factory()->creator()->create();
        $post = Post::factory()->for($creator, 'user')->inSeries('Chuỗi cũ', 2)->create();
        ...
        $this->assertNull($fresh->series_title);
    ...
    {
        Post::factory()->inSeries('Học Laravel từ đầu', 1)->create(['title' => 'Cài đặt môi trường']);
        $part2 = Post::factory()->inSeries('Học Laravel từ đầu', 2)->create(['title' => 'Routing cơ bản']);
        Post::factory()->inSeries('Học Laravel từ đầu', 3)->create(['title' => 'Eloquent nâng cao']);
    ...
    public function test_draft_parts_are_hidden_from_readers_but_shown_to_the_author(): void
    {
        ...
        $published = Post::factory()->for($author, 'user')->inSeries('Chuỗi A', 1)->create(['title' => 'Phần công khai']);
        Post::factory()->for($author, 'user')->draft()->inSeries('Chuỗi A', 2)->create(['title' => 'Phần nháp']);
    ...
    {
        Post::factory()->inSeries('Học Laravel từ đầu', 1)->create(['title' => 'Bài dài kỳ trên danh sách']);
